...
public class SubscriptionServiceImpl : ISubscriptionService
{
    ...
    public async Task<List<SubscriptionPlanDto>> GetPlansAsync(CancellationToken ct) => _m.Map<List<SubscriptionPlanDto>>(await _db.SubscriptionPlans.Include(p => p.IncludedServices).Include(p => p.WorkScopeRule).Include(p => p.SupportPolicy).Where(p => p.IsActive).ToListAsync(ct));
    ...
    public async Task<CustomerSubscriptionDto> CreateAsync(CreateSubscriptionRequest req, CancellationToken ct) { var s = new CustomerSubscription { CustomerId = req.CustomerId, PlanId = req.PlanId, StartDate = req.StartDate ?? DateTimeOffset.UtcNow, NextBillingDate = (req.StartDate ?? DateTimeOffset.UtcNow).AddMonths(1) }; _db.CustomerSubscriptions.Add(s); await _db.SaveChangesAsync(ct); return _m.Map<CustomerSubscriptionDto>(await _db.CustomerSubscriptions.Include(x => x.Plan).FirstAsync(x => x.Id == s.Id, ct)); }
